...
Downloading dPCR results of the ARGs and 16S/rpoB, currently in csv files in the same folder as the R Markdown. Each csv was used without renaming and has already been matched for sample location/date/solid content to calculate the resulting concentration on the spreadsheet for those that resulted in 3 or more positive droplets (detect). 

On the EVA sheet (corresponding to 16S/rpoB), empty wells have been taken out. Plate 4 rpoB plate had been mislabeled and was corrected (plate same as plate map except E11 -> A11). Plate 2 rpoB well H10 also mislabeled so corrected. 
...
#Selecting needed columns from each dataset to keep the data frame more manageable and renaming for ease of coding (VanB excluded since it had a double band)
data_02_ARG_mod <- select(data_01_ARG_raw, `Sample description 1`, 
                          `Accepted Droplets`, Positives, MergedWells, 
                          `Conc(copies/µL)...8`, plant, date, percent_solids, 
                          dry_mass, target, dilution, dry_gc_g, uci_68_dry, 
                          lci_68_dry, gc_ul_template, uci_68_gc_ul_template,
                          lci_68_gc_ul_template) %>%
...
## 1.3 QA/QC 
Following are the QA/QC criteria for all data:
...
Updated Mar 30, 2024: 
Original dimension of data_03_ARG_mod is 5098 x 17, of data_03_EVA_mod is 902 x 13, and of data_03_ctr_mod is 16 x 7. 
After QC1, data_04_ARG_pass is 5078 x 17 (20 measurements taken out), data_04_EVA_pass is 897 x 13 (5 measurements taken out), and data_04_ctr_pass is 16 x 8 (no measurements taken out). 

The 20 measurements taken out in ARG were: 
...
QAQC #2 was only conducted to EVA assays since the ARG assays were checked manually already that all NTCs were non detects. 
...
After QAQC, the dimensions for data_05_ARG_pass is 5043 x 18 (35 samples from #3 excluded) and for data_05_EVA_pass is 893 x 14 (4 samples from #2 excluded). 
...
#Selecting the bigger concentration since that happens to be the reruns 
for (i in 1:length(unique_id)) {
  i_sample <- unique_id[i]
  for (j in 1:length(unique_target)) {
    j_target <- unique_target[j]
    ...
    if (nrow(data_temp) > 1) {
      ...
      #If there's multiple minimum because the value is the same 
      if (length(i_min) > 1) {
...
data_08_ARG_mer <- merge(data_07_ARG_calc, data_02_met_mod, by = c("sample_id"), all.x = TRUE) %>% filter(!is.na(site))
...
After QA/QC, we have 443 samples and 4866 measurements that represent 163 plants in 40 states and 134 cities in the US. This is excluding the 2 samples from site 87 that should be excluded because they don't get exported (87-240519, 87-240521). 
...
ggplot(data_08_ARG_mer, aes(x = target, y = dry_gc_g, color = target)) + 
  ...
  scale_color_manual(values = c("#05386d","#54278f", "#08519c", "#fc9272", "#756bb1", "#3182bd", "#6baed6", "#9e9ac8", "#525252","#31a354","#9ecaef" )) + 
...
After QA/QC, we have 439 samples and 876 measurements that represent 163 plants in 40 states and 134 cities in the US. This is excluding the 2 samples from site 87 that should be excluded because they don't get exported (87-240519, 87-240521). 
...
data_09_EVA_16S <- data_08_EVA_16S[-c(247),]
data_09_EVA_rpoB <- data_08_EVA_rpoB[-c(22, 248, 24, 23),]
...
```
With Rosner's test on the log-transformed data, we identified one outlier (index = 247: 23-240517) that's the lowest point in 16S distribution and four outliers (index = 22: 107-240520, 248: 23-240517, 24: 107-240523, 23: 107-240522) that's the lowest four points in rpoB distribution. 

Taking those points out, it still does not make the distribution normal by shapiro-wilk, but we'll take them out for future analysis to prevent any impact to subsequent analysis.  
...
The linear regression between 16S vs rpoB shows that there is a significant relationship (p-value <2.2e-16) with R2 value of 0.78. The relationship can be described as y = 67.6x - 4.14e11 with both slope and intercept being statistically significant. 
...
#Order ARGs manually
data_10_mer$target <- factor(data_10_mer$target,
                             levels = c("CMY", "CTX-M", "KPC", "MecA", "NDM",
                                        "OXA-48", "TEM", "VIM",
                                        "MCR-1", "TetW", 
                                        "VanA"))
...
ggplot(data_10_mer, aes(x = target, y = conc_norm_S, color = target)) + 
  geom_jitter(aes(color = detect), size = 1, alpha = 0.7) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA, lwd = 0.7) +
  scale_y_continuous(trans = 'log10') + 
  scale_color_manual(values = c("#505050","#505050", "#505050", "#505050",
...
ggsave(Fig_DistBar, file = "Fig_DistBar.pdf", width = 9, height = 5)
...
2. Dataset related to socioeconomic vulnerability from the census  
3. Animal agriculture data that has number of chickens, cows, and hogs in the county matching each sewershed

*Note: for All_metadata_08_12_24.csv the following changes were made to the csv to match with EPIC data: 
1. First column of row numbers deleted
2. "Juneau and Burroughs" was changed to "Juneau" to match the name in other data sets 
3. District of Columbia's county and state was changed from Washington to District Of Columbia and District of Columbia
4. DuPage was changed to Dupage 
5. Punctuation was taken out from St. Joseph
6. Saint Mary's was changed to St Marys 

*Note: for EPIC, the rows "None of the above" and "Total" were deleted since they were irrelevant

```{r sec_import}
#County names for pulling EPIC data
sec_meta <- read_csv("All_metadata_08_12_24.csv")
sec_meta$County <- sub("^(.*?)( County| Municipality| Parish)?$", 
                       "\\1", sec_meta$County_Name)

#EPIC data
sec_epic <- read_csv("CountyCosmosPull_240815.csv") %>% 
  select(County, `Pen-Ceph1-4MRSA`, Colistin, Carbapenems, `Pen/CephALL`, 
         Tetracycline, Vancomycin, TotalEncounters) %>% 
  rename(County_State = County)

#Replace "10 or fewer" with 0
sec_epic[sec_epic == "10 or fewer"] <- "0"
sec_epic[c("Pen-Ceph1-4MRSA", "Colistin", "Carbapenems", "Pen/CephALL", 
         "Tetracycline", "Vancomycin")] <- lapply(sec_epic[c("Pen-Ceph1-4MRSA", 
                                                             "Colistin", 
                                                             "Carbapenems", 
                                                             "Pen/CephALL", 
                                                             "Tetracycline", 
...
#Split county name and state 
state_abbrev <- read_csv("states.csv")
sec_epic <- sec_epic %>% 
  separate(County_State, into = c("County", "State"), sep = ",") %>%
  mutate(County = str_to_title(County),
         State = str_trim(State) %>% str_to_upper(),
         State = recode(State, !!!setNames(state_abbrev$State, state_abbrev$`State Code`))) %>% 
  merge(state_abbrev, by = c("State")) %>%
  select(-`State Code`)

#Merge with site names 
sec_01_ABD_raw <- merge(sec_epic, sec_meta, 
                        by = c("State", "County"),
                        all.x = TRUE) %>%
  select(-County_FIPS, -Zipcode, -Population_Served, -County_Name, -`Pen-Ceph1-4MRSA`) %>%
  rename(site = Site_Name, 
         city = City, 
         state = State,
         blactam = `Pen/CephALL`)

#Take out punctuation from city and site names to match other datasets
sec_01_ABD_raw$site <- gsub("\\.", "", sec_01_ABD_raw$site)
sec_01_ABD_raw$city <- gsub("\\.", "", sec_01_ABD_raw$city)

#Normalize use by total encounters
sec_02_ABD_norm <- sec_01_ABD_raw %>% 
  mutate(Colistin_norm = Colistin/TotalEncounters,
         Carbapenems_norm = Carbapenems/TotalEncounters,
         blactam_norm = blactam/TotalEncounters,
         Tetracycline_norm = Tetracycline/TotalEncounters,
         Vancomycin_norm = Vancomycin/TotalEncounters)

#Various indicators including agriculture data
sec_01_ind_raw <- read_csv("secondary_data_arg.csv") %>% 
  rename(plant = name) %>%
  select(-Broilers, -Layers, -Pullets, -Roosters)
...
#Make column to indicate how many were detects 
data_11_sum$Detect <- NA

for (i in 1:length(data_11_sum$site)){
  i_site <- data_11_sum$site[i]
  i_targ <- data_11_sum$target[i]
  data_subset <- subset(data_10_mer, 
                        site == i_site & target == i_targ)
  if (all(data_subset$detect == "D")) {
    data_11_sum$Detect[i] <- "D"
    } else if (all(data_subset$detect == "ND")) {
      data_11_sum$Detect[i] <- "ND"
      } else {data_11_sum$Detect[i] <- "PD"}
}
...
```{r plot_sum}
#Order ARGs manually
data_11_sum$target <- factor(data_11_sum$target,
                             levels = c("CMY", "CTX-M", "KPC", "MecA", "NDM",
                                        "OXA-48", "TEM", "VIM",
                                        "MCR-1", "TetW", 
                                        "VanA"))
...
#Distribution when normalized by 16S 
Fig_DistBar_sum <-
ggplot(subset(data_11_sum, target != "VanB"), 
       aes(x = target, y = mean_ARG_S, color = target)) + 
  geom_jitter(aes(color = Detect), size = 1, alpha = 0.7, shape = 1) +
  geom_boxplot(alpha = 0, outlier.shape = NA, lwd = 0.5) +
  scale_y_continuous(trans = 'log10') + 
  scale_color_manual(values = c("#4c4c4c","#4c4c4c", "#4c4c4c", "#4c4c4c",
                                "#4c4c4c","#4c4c4c", "#4c4c4c", "#4c4c4c",
                                "#9a3030",
                                "#cf9e0f",
                                "#7B8D3C", 
                                "#b3b3b3","#e5c494","#6b4f2b")) + 
  ylab("Conc (-)") + 
  theme_bw() +
  labs(title = "ARG distribution normalized by 16S")

#Save figure as pdf file
ggsave(Fig_DistBar_sum, file = "Fig_DistBar_sum.pdf", width = 9, height = 5)
```
...
The animal agriculture data originally had various categories of cattle and chickens. For cattle, the following choices are available: cattle, excluding cows; cattle, cows; cattle, cows, beef; cattle, cows, milk; cattle, incl calves; cattle, on feed. Since cattle, incl calves is the broadest category, we're using that data. 

For chicken, the following data is available: chickens, broilers; chickens, layers; chickens, pullets, replacement; chickens, roosters. Since all chickens are relevant to our analysis, we've added the numbers up to get a total number of chickens. 
...
data_all_1 <- merge(data_11_sum, sec_02_ABD_norm, 
                    by = c("city", "state", "site"), 
                    all.x = TRUE) %>%
  select(-Colistin, -Carbapenems, -blactam, -Tetracycline, -Vancomycin)

data_all_2 <- merge(data_all_1, sec_01_ind_raw, by = c("plant"), all.x = TRUE)
```
...
data_all_2$zscore <- NA

#Assign z scores 
for (i in 1:length(data_all_2$plant)) {
  targ <- data_all_2$target[i]
  average = mean(subset(data_all_2, target == targ)$mean_ARG_S, na.rm = TRUE)
  std = sd(subset(data_all_2, target == targ)$mean_ARG_S, na.rm = TRUE)
  zscore <- (data_all_2$mean_ARG_S[i] - average)/std
  if(data_all_2$mean_ARG_S[i] < 3*std && data_all_2$mean_ARG_S[i] > -3*std) {
    data_all_2$zscore[i] <- zscore
  }
}

#Taking out outliers that was more than 3 standard deviations away
data_all_3 <- subset(data_all_2, !is.na(zscore))

#Assigning overall ARG burden 
data_burden_tot <- data_all_2 %>% 
...
#Colistin: mcr-1
data_burden_col <- data_all_2 %>% 
  subset(target == "MCR-1") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_col = sum(zscore)) 

#Vancomycin: vanA
data_burden_van <- data_all_2 %>% 
  subset(target == "VanA") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_van = sum(zscore)) 

#Tetracycline: tetW 
data_burden_tet <- data_all_2 %>% 
  subset(target == "TetW") %>%
  group_by(site, plant, city, state) %>%
...
data_burden_mer <- merge(data_burden, sec_01_ind_raw, by = c("plant"), all.x = TRUE) %>%
  merge(sec_02_ABD_norm, 
        by = c("city", "state", "site"), 
        all.x = TRUE) %>%
  select(-Colistin, -Carbapenems, -blactam, -Tetracycline, -Vancomycin)
```

ARG burden is also often expressed as raw sum of concentrations so relevant ARG concentrations were added. 

```{r totalConcAdded}
#Assigning overall ARG burden 
data_burden_add <- data_all_2 %>% 
  group_by(site, plant, city, state) %>%
  summarise(Burden_all = sum(mean_ARG_S)) 

#Combining class of antibiotics 
#Betalactamase: CMY, CTX-M, KPC, mecA, NDM, OXA-48, TEM, VIM
data_burden_bla_add <- data_all_2 %>% 
  subset(target == "CMY" | target == "CTX-M" | 
         target == "KPC" | target == "MecA" | target == "NDM" |
         target == "OXA-48" | target == "TEM" | target == "VIM") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_bla = sum(mean_ARG_S)) 

#Colistin: mcr-1
data_burden_col_add <- data_all_2 %>% 
  subset(target == "MCR-1") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_col = sum(mean_ARG_S)) 

#Vancomycin: vanA
data_burden_van_add <- data_all_2 %>% 
  subset(target == "VanA") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_van = sum(mean_ARG_S)) 

#Tetracycline: tetW 
data_burden_tet_add <- data_all_2 %>% 
  subset(target == "TetW") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_tet = sum(mean_ARG_S)) 

#Putting everything together
data_burden_add <- merge(data_burden_add, data_burden_bla_add, 
                     by = c("site", "plant", "city", "state"),
                     all = TRUE) %>%
  merge(data_burden_col_add,
        by = c("site", "plant", "city", "state"),
        all = TRUE) %>%
  merge(data_burden_van_add, 
        by = c("site", "plant", "city", "state"),
        all = TRUE) %>% 
  merge(data_burden_tet_add,
        by = c("site", "plant", "city", "state"),
        all = TRUE)

#Re-merge with secondary data
data_burden_add <- merge(data_burden_add, sec_01_ind_raw, by = c("plant"), all.x = TRUE) %>%
  merge(sec_02_ABD_norm, 
        by = c("city", "state", "site"), 
        all.x = TRUE) %>%
  select(-Colistin, -Carbapenems, -blactam, -Tetracycline, -Vancomycin)
...
write.csv(data_all_2, "data_all.csv", row.names = FALSE)
write.csv(data_all_3, "data_all_NoOutlier.csv", row.names = FALSE)
write.csv(data_burden_mer, "data_ARGburden.csv", row.names = FALSE)
write.csv(data_burden_add, "data_ARGburden_Conc.csv", row.names = FALSE)
```
