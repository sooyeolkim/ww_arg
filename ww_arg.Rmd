...
ggplot(data_10_mer, aes(x = target, y = conc_norm_S, color = target)) + 
  ...
  scale_y_continuous(trans = 'log10') + 
  scale_color_manual(values = c("#08519c","#08519c", "#08519c", "#08519c", "#08519c", "#08519c", "#08519c", "#08519c", "#fc9272","#525252","#31a354","#31a354" )) + 
  scale_shape_manual(values = c(19, 1)) +
  ylab("Conc (-)") + 
  theme_bw() +
...
sec_01_ind_raw <- read_csv("secondary_data_arg.csv") %>% 
  rename(plant = name) %>%
  select(-All_cattle, -Broiler_chicken, -Hogs,
         #Taking out themes since we have individual components as variables PROP_
...
sec_01_aa_raw <- read_csv("WWSCAN_sewershed_agriculture_v2.csv")
```
...
sec_01_aa_cow <- sec_01_aa_raw[grepl("CATTLE", sec_01_aa_raw$Data.Item, ignore.case = TRUE), ] %>% 
  ...
  select("name", "Data.Item", "Value_num")
...
sec_03_aa_wide <- sec_02_aa_filt %>% 
  select(plant, Data.Item, Value_num) %>% 
  pivot_wider(names_from = Data.Item, values_from = Value_num)
...
data_all_2 <- merge(data_all_1, sec_03_aa_wide, by = c("plant"), all.x = TRUE)
data_all_3 <- merge(data_all_2, sec_01_ind_raw, by = c("plant"), all.x = TRUE)
```
...
data_burden_mer <- merge(data_burden, sec_03_aa_wide, by = c("plant"), all.x = TRUE) %>%
  merge(sec_01_ind_raw, by = c("plant"), all.x = TRUE) %>%
...
#Assigning overall ARG burden 
data_burden_add <- data_all_3 %>% 
  group_by(site, plant, city, state) %>%
  summarise(Burden_all = sum(mean_ARG_S)) 

#Combining class of antibiotics 
#Betalactamase: CMY, CTX-M, KPC, mecA, NDM, OXA-48, TEM, VIM
data_burden_bla_add <- data_all_3 %>% 
  subset(target == "CMY" | target == "CTX-M" | 
         target == "KPC" | target == "MecA" | target == "NDM" |
         target == "OXA-48" | target == "TEM" | target == "VIM") %>%
  group_by(site, plant, city, state) %>%
...
#Colistin: mcr-1
data_burden_col_add <- data_all_3 %>% 
  subset(target == "MCR-1") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_col = sum(mean_ARG_S)) 

#Vancomycin: vanA, vanB
data_burden_van_add <- data_all_3 %>% 
  subset(target == "VanA" | target == "VanB") %>%
  group_by(site, plant, city, state) %>%
  summarise(Burden_van = sum(mean_ARG_S)) 

#Tetracycline: tetW 
data_burden_tet_add <- data_all_3 %>% 
  subset(target == "TetW") %>%
  group_by(site, plant, city, state) %>%
...
#Re-merge with secondary data
data_burden_add <- merge(data_burden_add, sec_03_aa_wide, by = c("plant"), all.x = TRUE) %>%
  merge(sec_01_ind_raw, by = c("plant"), all.x = TRUE) %>%
  merge(sec_02_ABD_norm, 
        by = c("city", "state", "site"), 
        all.x = TRUE) %>%
