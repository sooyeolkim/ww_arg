...
We'll also take out the measurement from UC Davis since its data isn't going to be used. 
...
After QA/QC, we have 443 samples and 5309 measurements that represent 163 plants in 40 states and 134 cities in the US. This is excluding the 2 samples from site 87 that Dora said should be excluded because they don't get exported (87-240519, 87-240521). 
...
2. Juneau and Burroughs was changed to Juneau 
...
*Note: for EPIC, the rows "None of the above" and "Total" were deleted 
...
ggplot(subset(data_11_sum, target != "VanB"), 
  ...
  geom_jitter(aes(color = Detect), size = 1, alpha = 0.7) +
  geom_boxplot(alpha = 0.5, outlier.shape = NA, lwd = 0.5) +
  scale_y_continuous(trans = 'log10') + 
